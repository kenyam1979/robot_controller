...
def generate_launch_description():
    ...
    imu_pkg_share = launch_ros.substitutions.FindPackageShare(package='wit_ros2_imu_fixed').find('wit_ros2_imu_fixed')
    ...
    lidar_launch = IncludeLaunchDescription(
        PathJoinSubstitution(
            [lidar_pkg_share, "laucnch", "sllidar_a1_launch.py"]))


    # Launch IMU sensor
    imu_launch = IncludeLaunchDescription(
        PathJoinSubstitution(
            [imu_pkg_share, "laucnch", "imu.launch.py"]))

    # Launch robot_state_publisher and localization
    robot_launch = IncludeLaunchDescription(
        PathJoinSubstitution(
            [description_pkg_share, "laucnch", "robot.launch.py"]))

    # Launch teleop
    teleop_launch = IncludeLaunchDescription(
        PathJoinSubstitution(
            [controller_pkg_share, "laucnch", "teleop_launch.py"]))
